...
const AdminNavigation = () => {
    const { isAuthenticating, profile } = react_redux_1.useSelector((state) => ({
...
const BasketItem = ({ product }) => {
    ...
    const onAddQty = () => {
        dispatch(basketActions_1.addQtyItem(product));
    };
    const onMinusQty = () => {
        if (product.quantity !== 1) {
            dispatch(basketActions_1.minusQtyItem(product));
        }
    };
    const onRemoveFromBasket = () => dispatch(basketActions_1.removeFromBasket(product));
    return (react_1.default.createElement(Container, null,
        ...
        react_1.default.createElement(Flexbox_1.default, { alignItems: "center" },
            react_1.default.createElement("h4", { className: "my-0" }, utils_1.displayMoney(product.price)),
...
const BasketToggle = ({ children }) => {
    const onClickToggle = useBasketClickToggle_1.default();
...
const Checkout = ({ basket, onCheckOut }) => {
    ...
    return (react_1.default.createElement(Flexbox_1.default, { background: "white", padding: "m-size", alignItems: "center", 
        ...
        react_1.default.createElement("div", null,
            ...
            react_1.default.createElement("h2", { className: "basket-total-amount" }, utils_1.displayMoney(utils_1.calculateTotal(basket.map((product) => product.price * product.quantity))))),
...
const Basket = () => {
    const { t } = react_i18next_1.useTranslation();
    const { isOpenModal, onOpenModal, onCloseModal } = hooks_1.useModal();
    const { basket, user } = react_redux_1.useSelector((state) => ({
        basket: state.basket,
        user: state.auth
    }));
    const history = react_router_dom_1.useHistory();
    const dispatch = react_redux_1.useDispatch();
    const didMount = hooks_1.useDidMount();
    react_1.useEffect(() => {
    ...
    const onClearBasket = () => {
        if (basket.length !== 0) {
            dispatch(basketActions_1.clearBasket());
        }
    };
...
const ColorChooser = ({ availableColors, onSelectedColorChange }) => {
    const [selectedColor, setSelectedColor] = react_1.useState('');
...
const Filters = ({ closeModal }) => {
    const { filter, isLoading, products } = react_redux_1.useSelector((state) => ({
        filter: state.filter,
        isLoading: state.app.loading,
        products: state.products.items
    }));
    const [field, setFilter] = react_1.useState({
        brand: filter.brand,
    ...
    const history = react_router_dom_1.useHistory();
    const didMount = hooks_1.useDidMount();
    const max = selector_1.selectMax(products);
    const min = selector_1.selectMin(products);
    react_1.useEffect(() => {
    ...
    const onApplyFilter = () => {
        ...
        if (isChanged) {
            dispatch(filterActions_1.applyFilter(field));
        }
        else {
            closeModal();
        }
    ...
    const onResetFilter = () => {
        ...
        if (filterFields.some((key) => !!filter[key])) {
            dispatch(filterActions_1.resetFilter());
        }
        else {
            closeModal();
        }
...
exports.default = react_router_dom_1.withRouter(Filters);
...
const Footer = () => {
    ...
    const matchRoute = react_router_dom_1.matchPath(pathname, {
...
const ImageLoader = ({ src, alt = '', className = 'image-loader' }) => {
    const loadedImages = {};
    // @ts-ignore
    const [loaded, setLoaded] = react_1.useState(loadedImages[src]);
    const onLoad = () => {
        // @ts-ignore
...
const Navigation = () => {
    const { t } = react_i18next_1.useTranslation();
    const navbar = react_1.useRef(null);
    const { pathname } = react_router_dom_1.useLocation();
    const store = react_redux_1.useSelector((state) => ({
...
const SearchBar = () => {
    ...
    const { filter, isLoading } = react_redux_1.useSelector((state) => ({
        filter: state.filter,
        isLoading: state.app.loading
    }));
    const searchbarRef = react_1.useRef(null);
    const history = react_router_dom_1.useHistory();
    const dispatch = react_redux_1.useDispatch();
    ...
    const onClearRecent = () => {
        dispatch(filterActions_1.clearRecentSearch());
    };
    return (react_1.default.createElement(react_1.default.Fragment, null,
        react_1.default.createElement("div", { className: "searchbar", ref: searchbarRef },
            ...
            filter.recent.length !== 0 && (react_1.default.createElement("div", { className: "searchbar-recent" },
                ...
                filter.recent.map((item, index) => (react_1.default.createElement("div", { className: "searchbar-recent-wrapper", key: `search-${item}-${index}` },
                    ...
                    react_1.default.createElement("span", { className: "searchbar-recent-button text-subtle", onClick: () => dispatch(filterActions_1.removeSelectedRecent(item)), role: "presentation" }, "X")))))))));
};
...
const SocialLogin = ({ isLoading }) => {
    ...
    const onSignInWithGoogle = () => {
        dispatch(authActions_1.signInWithGoogle());
    };
    const onSignInWithFacebook = () => {
        dispatch(authActions_1.signInWithFacebook());
    };
    const onSignInWithGithub = () => {
        dispatch(authActions_1.signInWithGithub());
    };
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const react_1 = __importDefault(__webpack_require__(67294));
const CustomInput = ({ disabled, field, meta, label, placeholder }) => {
    const touched = meta?.touched;
    const { error } = meta;
    return (react_1.default.createElement("div", { className: "input-group" },
        touched && error ? (react_1.default.createElement("span", { className: "label-input label-error" }, error)) : (react_1.default.createElement("label", { className: "label-input", htmlFor: field.name }, label)),
        react_1.default.createElement("input", { ...field, type: "text", disabled: disabled, id: field.name, placeholder: placeholder, className: `input-form ${touched && error && 'input-error'}` })));
};
...
const ProductAppliedFilters = ({ filteredProductsCount }) => {
    const filter = react_redux_1.useSelector((state) => state.filter, react_redux_1.shallowEqual);
    ...
    const onRemoveKeywordFilter = () => {
        dispatch(filterActions_1.applyFilter({ keyword: '' }));
    };
    const onRemovePriceRangeFilter = () => {
        dispatch(filterActions_1.applyFilter({ minPrice: 0, maxPrice: 0 }));
    };
    const onRemoveBrandFilter = () => {
        dispatch(filterActions_1.applyFilter({ brand: '' }));
    };
    const onRemoveSortFilter = () => {
        dispatch(filterActions_1.applyFilter({ sortBy: '' }));
    };
...
const ProductList = ({ products, filteredProducts, isLoading, requestStatus = null, children }) => {
    const [isFetching, setFetching] = react_1.useState(false);
    const dispatch = react_redux_1.useDispatch();
    const fetchProducts = () => {
        setFetching(true);
        dispatch(productActions_1.getProducts(products.lastRefKey));
    };
    react_1.useEffect(() => {
        ...
        window.scrollTo(0, 0);
        return () => dispatch(miscActions_1.setLoading(false));
    }, []);
    react_1.useEffect(() => {
        setFetching(false);
    }, [products.lastRefKey]);
...
const ProductSearch = () => {
    const history = react_router_dom_1.useHistory();
    const { productsLength, filter, products, isLoading } = react_redux_1.useSelector((state) => ({
        filter: state.filter,
        products: state.products.items,
        isLoading: state.app.loading,
    ...
    const dispatch = react_redux_1.useDispatch();
    const searchInput = react_1.useRef(null);
    let input = '';
    react_1.useEffect(() => {
    ...
    const onSearchChange = (e) => {
        ...
        if (val === '' && productsLength !== 0) {
            dispatch(filterActions_1.setTextFilter(val));
            history.push('/');
        }
    };
    const onKeyUp = (e) => {
        if (e.keyCode === 13 && productsLength !== 0) {
            dispatch(filterActions_1.setTextFilter(input));
            history.push('/');
        }
    };
                ...
                react_1.default.createElement("div", { className: "pill-wrapper", key: `${item}${index}` },
                    react_1.default.createElement("div", { className: "pill padding-right-l" },
                        react_1.default.createElement("h5", { className: "pill-content margin-0", onClick: () => {
                                dispatch(filterActions_1.setTextFilter(item));
                                history.push('/');
                            }, role: "presentation" }, item),
                        react_1.default.createElement("div", { className: "pill-remove", onClick: () => dispatch(filterActions_1.removeSelectedRecent(item)), role: "presentation" },
...
const styleProps = styled_system_1.compose(styled_system_1.layout, styled_system_1.space, styled_system_1.position, styled_system_1.flex, styled_system_1.grid, styled_system_1.border, styled_system_1.flexbox, styled_system_1.background);
...
const styled_components_1 = __importDefault(__webpack_require__(29958));
const Box_1 = __importDefault(__webpack_require__(31285));
const Flexbox = styled_components_1.default(Box_1.default).attrs(() => ({
    display: 'flex'
})) ``;
...
const Container = styled_components_1.default(Box_1.default).attrs(() => ({
    as: 'main'
})) `
  width: 100%;
  min-height: 100vh;
    ...
    return (react_1.default.createElement(react_loading_skeleton_1.SkeletonTheme, { color: "#e1e1e1", highlightColor: "#f2f2f2" },
        react_1.default.createElement(Container, { onClick: onClickItem, role: "presentation" },
            ...
            react_1.default.createElement(InfoWrapper, null,
                ...
                react_1.default.createElement("p", { className: "text-subtle text-italic" }, product.price ? (utils_1.displayMoney(product.price)) : (react_1.default.createElement(react_loading_skeleton_1.default, { width: 40 })))))));
};
const Container = styled_components_1.default.div `
  width: 100%;
  max-height: 30rem;
...
const Box_1 = __importDefault(__webpack_require__(31285));
const styled_components_1 = __importDefault(__webpack_require__(29958));
const Separator = styled_components_1.default(Box_1.default) `
  width: 100%;
  height: 1px;
...
const useBasket = () => {
    const { t } = react_i18next_1.useTranslation();
    const { basket } = react_redux_1.useSelector((state) => ({
        basket: state.basket
    }));
    const dispatch = react_redux_1.useDispatch();
    ...
    const addToBasket = (product) => {
        if (isItemOnBasket(product.id, product.variant)) {
            dispatch(basketActions_1.removeFromBasket(product));
            utils_1.displayActionMessage(t('common.toasts.removeProductFromBasket'), 'info');
        }
        else {
            dispatch(basketActions_1.addToBasket(product));
            utils_1.displayActionMessage(t('common.toasts.addedProductToBasket'), 'success');
        }
    };
...
const useDidMount = (initState = false) => {
    const [didMount, setDidMount] = react_1.useState(initState);
    react_1.useEffect(() => {
        setDidMount(true);
        return () => {
            setDidMount(false);
...
const useFeaturedProducts = (itemsCount) => {
    const [featuredProducts, setFeaturedProducts] = react_1.useState([]);
    const [isLoading, setLoading] = react_1.useState(false);
    const [error, setError] = react_1.useState('');
    const didMount = hooks_1.useDidMount(true);
...
const useFileHandler = (initState) => {
    const [imageFile, setImageFile] = react_1.useState(initState);
    const [isFileLoading, setFileLoading] = react_1.useState(false);
            ...
            Array.from(event.target.files).forEach((file) => {
                ...
                reader.addEventListener('load', (e) => {
                    setImageFile((oldFiles) => ({
                        ...
                        [name]: [
                            ...oldFiles[name],
                            { file, url: e.target.result, id: uuid_1.v4() }
                        ]
                    }));
                });
...
const react_1 = __webpack_require__(67294);
const useModal = () => {
    const [isOpenModal, setModalOpen] = react_1.useState(false);
    const onOpenModal = () => {
        setModalOpen(true);
    };
    const onCloseModal = () => {
...
const firebase_1 = __importDefault(__webpack_require__(99826));
const useProduct = (id) => {
    const storeProduct = react_redux_1.useSelector((state) => state.products.items.find((item) => item.id === id));
    const [product, setProduct] = react_1.useState(storeProduct);
    const [isLoading, setLoading] = react_1.useState(false);
...
const useRecommendedProducts = (itemsCount) => {
    const [recommendedProducts, setRecommendedProducts] = react_1.useState([]);
    const [isLoading, setLoading] = react_1.useState(false);
    const [error, setError] = react_1.useState('');
    const didMount = hooks_1.useDidMount(true);
...
const { store, persistor } = store_1.default();
const root = document.getElementById('app');
react_dom_1.render(react_1.default.createElement(common_1.Preloader, null), root);
firebase_1.default.auth.onAuthStateChanged((user) => {
    if (user) {
        store.dispatch(authActions_1.onAuthStateSuccess(user));
    }
    else {
        store.dispatch(authActions_1.onAuthStateFail('Failed to authenticate'));
    }
    react_dom_1.render(react_1.default.createElement(App_1.default, { store: store, persistor: persistor }), root);
});
...
const Image = yup_1.object().shape({
    id: yup_1.string().required('Image powinno mieć id.'),
    url: yup_1.string().required('Image powinno mieć url.')
});
const product = yup_1.object().shape({
    name: yup_1.string()
    ...
    description: yup_1.string().required('Description is required.'),
    sizes: yup_1.array().of(yup_1.string()).min(1, 'Please enter a size for this product.'),
    isFeatured: yup_1.boolean(),
    isRecommended: yup_1.boolean(),
    gallery: yup_1.array().of(Image),
    quantity: yup_1.number().integer()
});
...
function* handleError(e) {
    ...
    yield effects_1.put(miscActions_1.setAuthenticating(false));
    switch (e.code) {
        case 'auth/network-request-failed':
            yield effects_1.put(miscActions_1.setAuthStatus({
                ...obj,
        ...
        case 'auth/wrong-password':
            yield effects_1.put(miscActions_1.setAuthStatus({ ...obj, message: 'Incorrect email or password' }));
            break;
        ...
        case 'auth/reset-password-error':
            ...
            break;
        default:
            yield effects_1.put(miscActions_1.setAuthStatus({ ...obj, message: e.message }));
            break;
    }
}
function* initRequest() {
    yield effects_1.put(miscActions_1.setAuthenticating());
    yield effects_1.put(miscActions_1.setAuthStatus());
}
function* authSaga({ type, payload }) {
    switch (type) {
        case constants_1.SIGNIN:
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.signIn, payload.email, payload.password);
            }
            catch (e) {
                yield handleError(e);
            }
            break;
        case constants_1.SIGNIN_WITH_GOOGLE:
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.signInWithGoogle);
            }
            catch (e) {
                yield handleError(e);
            }
            break;
        case constants_1.SIGNIN_WITH_FACEBOOK:
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.signInWithFacebook);
            }
            catch (e) {
                yield handleError(e);
            }
            break;
        case constants_1.SIGNIN_WITH_GITHUB:
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.signInWithGithub);
            }
            catch (e) {
                yield handleError(e);
            }
        ...
        case constants_1.SIGNUP:
            try {
                yield initRequest();
                const ref = yield effects_1.call(firebase_1.default.createAccount, payload.email, payload.password);
                const user = {
                    user: {
                        email: payload.email
                    },
                ...
                yield effects_1.call(firebase_1.default.addUser, ref.user.uid, user);
                yield effects_1.put(profileActions_1.setProfile(user));
                yield effects_1.put(miscActions_1.setAuthenticating(false));
            }
            catch (e) {
                yield handleError(e);
        ...
        case constants_1.SIGNOUT: {
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.signOut);
                yield effects_1.put(basketActions_1.clearBasket());
                yield effects_1.put(profileActions_1.clearProfile());
                yield effects_1.put(filterActions_1.resetFilter());
                yield effects_1.put(authActions_1.signOutSuccess());
                yield effects_1.put(miscActions_1.setAuthenticating(false));
                yield effects_1.call(AppRouter_1.history.push, routes_1.SIGNIN);
            }
            catch (e) {
                console.log(e);
            }
        ...
        case constants_1.RESET_PASSWORD: {
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.passwordReset, payload);
                yield effects_1.put(miscActions_1.setAuthStatus({
                    success: true,
                    type: 'reset',
        ...
        case constants_1.ON_AUTHSTATE_SUCCESS: {
            const snapshot = yield effects_1.call(firebase_1.default.getUser, payload.uid);
            if (snapshot.data()) {
                // if user exists in database
                const user = snapshot.data();
                yield effects_1.put(profileActions_1.setProfile(user));
                yield effects_1.put(contactDetailsActions_1.setContactDetailsLocaly(user.user));
                const userOrders = yield effects_1.call(firebase_1.default.getUserOrders, payload.uid);
                yield effects_1.put(ordersActions_1.setOrdersItems(userOrders));
                // yield put(setContactDetailsLocaly(user.contactDetails))
                // yield put(setBasketItems(user.basket))
                yield effects_1.put(authActions_1.signInSuccess({
                    id: payload.uid,
                    role: user.role,
        ...
        case constants_1.SET_AUTH_PERSISTENCE: {
            try {
                yield effects_1.call(firebase_1.default.setAuthPersistence);
            }
            catch (e) {
                console.log(e);
            }
...
function* rootSaga() {
    ...
    yield effects_1.takeLatest([ACTION.UPDATE_EMAIL, ACTION.UPDATE_PROFILE, ACTION.ADD_PREFERRED_DELIVERY], profileSaga_1.default);
    yield effects_1.takeLatest([ACTION.ADD_ORDER_REMOTELY], orderSaga_1.default);
    yield effects_1.takeLatest([ACTION.SET_SHIPPING_LOCALY], shippingSaga_1.default);
}
...
const firebase_1 = __importDefault(__webpack_require__(99826));
function* initRequest() {
    yield effects_1.put(miscActions_1.setLoading(true));
    yield effects_1.put(miscActions_1.setRequestStatus(null));
}
function* handleError(e) {
    yield effects_1.put(miscActions_1.setLoading(false));
    yield effects_1.put(miscActions_1.setRequestStatus(e?.message || 'Failed to fetch products'));
    console.log('ERROR: ', e);
}
function* handleAction(location, message, status) {
    if (location)
        yield effects_1.call(AppRouter_1.history.push, location);
    yield effects_1.call(utils_1.displayActionMessage, message, status);
}
    ...
    switch (type) {
        case constants_1.ADD_ORDER_REMOTELY: {
            try {
                yield initRequest();
                //
                const orderId = yield effects_1.call(firebase_1.default.generateKey);
                ...
                yield effects_1.call(firebase_1.default.addOrder, orderId, order);
                //
                const profileOrder = order;
                yield effects_1.call(firebase_1.default.addOrderToProfile, userId, orderId, profileOrder);
                //
                const userOrders = yield effects_1.call(firebase_1.default.getUserOrders, userId);
                yield effects_1.put(ordersActions_1.setOrdersItems(userOrders));
                //
                yield effects_1.put(basketActions_1.clearBasket());
            }
    ...
    switch (type) {
        case constants_1.GET_PRODUCTS:
            try {
                ...
                const result = yield effects_1.call(firebase_1.default.getProducts, payload);
                if (result.products.length === 0) {
                    handleError('No items found.');
                }
                else {
                    yield effects_1.put(productActions_1.getProductsSuccess({
                    ...
                    yield effects_1.put(miscActions_1.setRequestStatus(''));
                }
        ...
        case constants_1.ADD_PRODUCT: {
            try {
                ...
                const key = yield effects_1.call(firebase_1.default.generateKey);
                let images = [];
                if (gallery.length !== 0) {
                    const imageKeys = yield effects_1.all(gallery.map(() => firebase_1.default.generateKey));
                    const imageUrls = yield effects_1.all(gallery.map((img, i) => firebase_1.default.storeImage(imageKeys[i](), 'products', img.file)));
                ...
                yield effects_1.call(firebase_1.default.addProduct, key, product);
                yield effects_1.put(productActions_1.addProductSuccess({
                    id: key,
                    ...product
                }));
        ...
        case constants_1.EDIT_PRODUCT: {
            try {
                ...
                if (gallery.length > 1) {
                    ...
                    const imageKeys = yield effects_1.all(newUploads.map(() => firebase_1.default.generateKey));
                    const imageUrls = yield effects_1.all(newUploads.map((img, i) => firebase_1.default.storeImage(imageKeys[i](), 'products', img.file)));
                ...
                yield effects_1.call(firebase_1.default.editProduct, payload.id, newUpdates);
                yield effects_1.put(productActions_1.editProductSuccess({
                    id: payload.id,
                    updates: newUpdates
                }));
        ...
        case constants_1.REMOVE_PRODUCT: {
            try {
                yield initRequest();
                yield effects_1.call(firebase_1.default.removeProduct, payload);
                yield effects_1.put(productActions_1.removeProductSuccess(payload));
                yield effects_1.put(miscActions_1.setLoading(false));
        ...
        case constants_1.SEARCH_PRODUCT: {
            try {
                yield initRequest();
                // clear search data
                yield effects_1.put(productActions_1.clearSearchState());
                const state = yield effects_1.select();
                const result = yield effects_1.call(firebase_1.default.searchProducts, payload.searchKey);
                if (result.products.length === 0) {
                    yield handleError({ message: 'No product found.' });
                    yield effects_1.put(productActions_1.clearSearchState());
                }
                else {
                    yield effects_1.put(productActions_1.searchProductSuccess({
                        products: result.products,
                        lastKey: result.lastKey
                            ? result.lastKey
...
function* profileSaga({ type, payload }) {
    switch (type) {
        case constants_1.ADD_PREFERRED_DELIVERY:
            try {
                yield effects_1.put(preferredDeliveryActions_1.setPreferredDelivery(payload.deliveryType));
                yield effects_1.call(firebase_1.default.updateProfile, payload.id, {
        ...
        case constants_1.UPDATE_EMAIL: {
            try {
                yield effects_1.put(miscActions_1.setLoading(false));
                yield effects_1.call(firebase_1.default.updateEmail, payload.password, payload.newEmail);
                yield effects_1.put(miscActions_1.setLoading(false));
                yield effects_1.call(AppRouter_1.history.push, '/profile');
                yield effects_1.call(utils_1.displayActionMessage, 'Email Updated Successfully!', 'success');
            }
            catch (e) {
        ...
        case constants_1.UPDATE_PROFILE: {
            try {
                ...
                if (email && password && email !== state.profile.email) {
                    yield effects_1.call(firebase_1.default.updateEmail, password, email);
                }
                if (avatarFile || bannerFile) {
                    const bannerURL = bannerFile
                        ? yield effects_1.call(firebase_1.default.storeImage, state.auth.id, 'banner', bannerFile)
                        : payload.updates.banner;
                    const avatarURL = avatarFile
                        ? yield effects_1.call(firebase_1.default.storeImage, state.auth.id, 'avatar', avatarFile)
                    ...
                    yield effects_1.call(firebase_1.default.updateProfile, state.auth.id, updates);
                    yield effects_1.put(profileActions_1.updateProfileSuccess(updates));
                }
                else {
                    yield effects_1.call(firebase_1.default.updateProfile, state.auth.id, payload.updates);
                    yield effects_1.put(profileActions_1.updateProfileSuccess(payload.updates));
                }
                yield effects_1.put(miscActions_1.setLoading(false));
                yield effects_1.call(AppRouter_1.history.push, routes_1.ACCOUNT);
                yield effects_1.call(utils_1.displayActionMessage, 'Profile Updated Successfully!', 'success');
            }
            catch (e) {
                yield effects_1.put(miscActions_1.setLoading(false));
                if (e.code === 'auth/wrong-password') {
                    yield effects_1.call(utils_1.displayActionMessage, 'Wrong password, profile update failed :(', 'error');
                }
                else {
                    yield effects_1.call(utils_1.displayActionMessage, `:( Failed to update profile. ${e.message ? e.message : ''}`, 'error');
                }
            }
            break;
        }
...
const store = redux_1.createStore(redux_persist_1.persistCombineReducers(authPersistConfig, reducers_1.default), composeEnhancer(redux_1.applyMiddleware(sagaMiddleware)));
...
exports.default = react_redux_1.connect(mapStateToProps)(AdminRoute);
...
const AppRouter = () => {
    const store = react_redux_1.useSelector((stor) => ({
        ...stor
    }));
    console.log('+', store);
...
exports.default = react_redux_1.connect(mapStateToProps)(PublicRoute);
...
const ForgotPassword = () => {
    const { authStatus, isAuthenticating } = react_redux_1.useSelector((state) => ({
        isAuthenticating: state.app.isAuthenticating,
        authStatus: state.app.authStatus
    }));
    const dispatch = react_redux_1.useDispatch();
    const didMount = hooks_1.useDidMount();
    const [forgotPWStatus, setForgotPWStatus] = react_1.useState({});
    const [isSendingForgotPWRequest, setIsSending] = react_1.useState(false);
    const [field, setField] = react_1.useState({});
    hooks_1.useScrollTop();
    hooks_1.useDocumentTitle('Forgot Password | Salinaka');
    react_1.useEffect(() => {
        if (didMount) {
    ...
    const onSubmitEmail = () => {
        if (!!field.email && !field.error) {
            dispatch(authActions_1.resetPassword(field.email));
        }
    };
    return (react_1.default.createElement("div", { className: "forgot_password" },
        ...
        react_1.default.createElement("h3", null, "Forgot Your Password?"),
        react_1.default.createElement("p", null, "Enter your email address and we will send you a password reset email."),
        react_1.default.createElement("br", null),
        react_1.default.createElement("input", { field: "email", required: true, className: "input-form", label: "* Email", maxLength: 40, onChange: onEmailChange, placeholder: "Enter your email", readOnly: isSendingForgotPWRequest || authStatus?.success, type: "email", style: { width: '100%' } }),
        react_1.default.createElement("br", null),
        react_1.default.createElement("br", null),
...
const Home = () => {
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useDocumentTitle('Salinaka | Home');
    hooks_1.useScrollTop();
    const { featuredProducts, fetchFeaturedProducts, isLoading: isLoadingFeatured, error: errorFeatured } = hooks_1.useFeaturedProducts(6);
    const { recommendedProducts, fetchRecommendedProducts, isLoading: isLoadingRecommended, error: errorRecommended } = hooks_1.useRecommendedProducts(6);
    return (react_1.default.createElement(Page_1.default, null,
...
const selector_1 = __webpack_require__(53143);
const Shop = () => {
    hooks_1.useDocumentTitle('Shop | Salinaka');
    hooks_1.useScrollTop();
    const store = react_redux_1.useSelector((state) => ({
        filteredProducts: selector_1.selectFilter(state.products.items, state.filter),
...
const useForm = () => {
    const dispatch = react_redux_1.useDispatch();
    const getSchema = () => yup_1.object().shape({
        email: yup_1.string()
    ...
    const submitForm = (form) => {
        dispatch(authActions_1.signIn(form));
    };
...
const SignIn = () => {
    ...
    const dispatch = react_redux_1.useDispatch();
    const { getInitialValues, getSchema, submitForm } = form_1.default();
    hooks_1.useScrollTop();
    hooks_1.useDocumentTitle('Sign In | Salinaka');
    react_1.useEffect(() => () => {
        dispatch(miscActions_1.setAuthStatus());
        dispatch(miscActions_1.setAuthenticating(false));
    }, []);
...
const SignUp = () => {
    const history = react_router_dom_1.useHistory();
    const { t } = react_i18next_1.useTranslation();
    const { isAuthenticating, authStatus } = react_redux_1.useSelector((state) => ({
        isAuthenticating: state.app.isAuthenticating,
        authStatus: state.app.authStatus
    }));
    const dispatch = react_redux_1.useDispatch();
    const { submitForm, getInitialValues, getSchema } = form_1.default();
    hooks_1.useScrollTop();
    hooks_1.useDocumentTitle('Sign Up | Salinaka');
    react_1.useEffect(() => () => {
        dispatch(miscActions_1.setAuthStatus());
        dispatch(miscActions_1.setAuthenticating(false));
    }, []);
        ...
        !authStatus?.success && (react_1.default.createElement(react_1.default.Fragment, null,
            ...
            react_1.default.createElement("div", { className: `auth ${authStatus?.message && !authStatus?.success && 'input-error'}` },
                react_1.default.createElement("div", { className: "auth-main" },
                    ...
                    react_1.default.createElement(formik_2.Formik, { initialValues: getInitialValues(), validateOnChange: true, validationSchema: getSchema(), onSubmit: submitForm }, () => (react_1.default.createElement(formik_2.Form, null,
                        react_1.default.createElement("div", { className: "auth-field" },
                            react_1.default.createElement(formik_2.Field, { name: "email", type: "email", disabled: isAuthenticating }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, label: `* ${t('signUpPage.email.label')}`, placeholder: t('signUpPage.email.placeholder') })))),
                        react_1.default.createElement("div", { className: "auth-field" },
                            react_1.default.createElement(formik_2.Field, { disabled: isAuthenticating, name: "password", type: "password" }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, label: `* ${t('signUpPage.password.label')}`, placeholder: t('signUpPage.password.placeholder') })))),
                        react_1.default.createElement("br", null),
...
const Profile = () => {
    const { t } = react_i18next_1.useTranslation();
    const { profile, contactDetails, custmerType } = react_redux_1.useSelector((state) => ({
        profile: state.profile,
    ...
    return (react_1.default.createElement(Flexbox_1.default, { flexDirection: "column", marginX: "auto" },
        ...
        react_1.default.createElement("div", { className: "user-profile-details" },
            ...
            react_1.default.createElement(SingleDetail_1.default, { label: t('profilePage.account.fields.dateJoined'), value: utils_1.displayDate(profile.dateJoined) }))));
};
exports.default = react_router_dom_1.withRouter(Profile);
...
const Profile = react_1.lazy(() => Promise.resolve().then(() => __importStar(__webpack_require__(8018))));
const UserOrdersTab = react_1.lazy(() => Promise.resolve().then(() => __importStar(__webpack_require__(29687))));
const UserAccount = () => {
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useScrollTop();
    hooks_1.useDocumentTitle('My Account | Salinaka');
...
const authActions_1 = __webpack_require__(17051);
const UserNav = () => {
    const { profile, isAuthenticating } = react_redux_1.useSelector((state) => ({
        profile: state.profile,
        isAuthenticating: state.app.isAuthenticating
    }));
    const userNav = react_1.useRef(null);
    const dispatch = react_redux_1.useDispatch();
    ...
    return isAuthenticating ? (react_1.default.createElement("div", { className: "user-nav" },
        ...
        react_1.default.createElement("div", { className: "user-nav-sub" },
            ...
            react_1.default.createElement("h6", { className: "user-nav-sub-link margin-0 d-flex", onClick: () => dispatch(authActions_1.signOut()), role: "presentation" },
                "Sign Out",
...
exports.default = react_router_dom_1.withRouter(UserNav);
...
const UserTab = (props) => {
    ...
    const [activeTab, setActiveTab] = react_1.useState(children[0].props.index || 0);
...
const EditProfile = () => {
    hooks_1.useDocumentTitle('Edit Account | Salinaka');
    hooks_1.useScrollTop();
    const modal = hooks_1.useModal();
    const dispatch = react_redux_1.useDispatch();
    react_1.useEffect(() => () => {
        dispatch(miscActions_1.setLoading(false));
    }, []);
    const { profile, auth, isLoading } = react_redux_1.useSelector((state) => ({
        profile: state.profile,
        auth: state.auth,
        isLoading: state.app.loading
    }));
    ...
    const { imageFile, isFileLoading, onFileChange } = hooks_1.useFileHandler({
        avatar: {},
        banner: {}
    });
    const update = (form, credentials = {}) => {
        dispatch(profileActions_1.updateProfile({
            updates: {
...
const react_router_dom_1 = __webpack_require__(77856);
const productActions_1 = __webpack_require__(82972);
const ProductForm = react_1.lazy(() => Promise.resolve().then(() => __importStar(__webpack_require__(6475))));
const AddProduct = () => {
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useScrollTop();
    hooks_1.useDocumentTitle('Add New Product | Salinaka');
    const isLoading = react_redux_1.useSelector((state) => state.app.loading);
    const dispatch = react_redux_1.useDispatch();
    const onSubmit = (product) => {
        dispatch(productActions_1.addProduct(product));
    };
...
exports.default = react_router_dom_1.withRouter(AddProduct);
...
const react_1 = __importDefault(__webpack_require__(67294));
const Dashboard = () => {
    hooks_1.useDocumentTitle('Welcome | Admin Dashboard');
    hooks_1.useScrollTop();
    return (react_1.default.createElement("div", { className: "loader" },
...
const EditProduct = ({ match }) => {
    hooks_1.useDocumentTitle('Edit Product | Salinaka');
    hooks_1.useScrollTop();
    const { product, error, isLoading } = hooks_1.useProduct(match.params.id);
    const dispatch = react_redux_1.useDispatch();
    const onSubmitForm = (updates) => {
        dispatch(productActions_1.editProduct(product.id, updates));
    };
    return (react_1.default.createElement("div", { className: "product-form-container" },
...
exports.default = react_router_dom_1.withRouter(EditProduct);
...
const selector_1 = __webpack_require__(53143);
const components_1 = __webpack_require__(14510);
const Table_1 = __importDefault(__webpack_require__(57264));
const Products = () => {
    hooks_1.useDocumentTitle('Product List | Salinaka Admin');
    hooks_1.useScrollTop();
    const store = react_redux_1.useSelector((state) => ({
        filteredProducts: selector_1.selectFilter(state.products.items, state.filter),
        requestStatus: state.app.requestStatus,
...
exports.default = react_router_dom_1.withRouter(Products);
...
const ProductForm = ({ product, onSubmit, isLoading }) => {
    ...
    const { imageFile, isFileLoading, onFileChange, removeImage } = hooks_1.useFileHandler({
        gallery: product?.gallery || []
    });
...
const ProductItem = ({ product }) => {
    const dispatch = react_redux_1.useDispatch();
    const history = react_router_dom_1.useHistory();
    const productRef = react_1.useRef(null);
    const onClickEdit = () => {
    ...
    const onConfirmDelete = () => {
        dispatch(productActions_1.removeProduct(product.id));
        utils_1.displayActionMessage('Item successfully deleted');
    ...
    return (react_1.default.createElement(react_loading_skeleton_1.SkeletonTheme, { color: "#e1e1e1", highlightColor: "#f2f2f2" },
        react_1.default.createElement("div", { className: `item item-products ${!product.id && 'item-loading'}`, ref: productRef },
            react_1.default.createElement("div", { className: "grid grid-count-6" },
                ...
                react_1.default.createElement("div", { className: "grid-col" },
                    react_1.default.createElement("span", null, product.price ? (utils_1.displayMoney(product.price)) : (react_1.default.createElement(react_loading_skeleton_1.default, { width: 30 })))),
                react_1.default.createElement("div", { className: "grid-col" },
                    react_1.default.createElement("span", null, product.dateAdded ? (utils_1.displayDate(product.dateAdded)) : (react_1.default.createElement(react_loading_skeleton_1.default, { width: 30 }))))),
...
exports.default = react_router_dom_1.withRouter(ProductItem);
...
const Result = () => {
    const { orders } = react_redux_1.useSelector((store) => ({
        orders: store.orders
    }));
    const { t } = react_i18next_1.useTranslation();
    const history = react_router_dom_1.useHistory();
...
const Wrapper_1 = __importDefault(__webpack_require__(91747));
const Step1 = ({ total }) => {
    const { basket } = react_redux_1.useSelector((store) => ({
        basket: store.basket
    }));
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useDocumentTitle('Check Out Step 1 | Salinaka');
    hooks_1.useScrollTop();
    const isBasketEmpty = basket.length === 0;
    return (react_1.default.createElement(Checkout_1.default, null,
...
exports.default = WithCheckout_1.default(Step1);
...
const Form = ({ customerType }) => {
    const { t } = react_i18next_1.useTranslation();
    const { values, resetForm, initialValues } = formik_2.useFormikContext();
    const { hasPreviousOrder } = react_redux_1.useSelector((store) => ({
            ...
            react_1.default.createElement("div", { className: "checkout-fieldset" },
                react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_2.Field, { name: "fullname", type: "text" }, (props) => {
                        ...
                        return (react_1.default.createElement(formik_1.CustomInput, { ...props, style: { textTransform: 'capitalize' }, disabled: hasPreviousOrder && initialValues.fullname, label: getRequiredOrNot_1.default(t(`checkoutPage.step2.form.${base}.label`), true), placeholder: t(`checkoutPage.step2.form.${base}.placeholder`) }));
                    })),
                customerType === 'COMPANY' && (react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_2.Field, { name: "nip", type: "text" }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, disabled: hasPreviousOrder && initialValues.nip, label: getRequiredOrNot_1.default(t('checkoutPage.step2.form.nip.label'), true), placeholder: t('checkoutPage.step2.form.nip.placeholder') })))))),
            react_1.default.createElement("div", { className: "checkout-fieldset" },
                react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_2.Field, { name: "street", type: "text" }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, disabled: hasPreviousOrder && initialValues.street, label: getRequiredOrNot_1.default(t('checkoutPage.step2.form.street.label'), true), placeholder: t('checkoutPage.step2.form.street.placeholder') }))))),
            react_1.default.createElement("div", { className: "checkout-fieldset" },
                react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_2.Field, { name: "postCode", type: "text" }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, disabled: hasPreviousOrder && initialValues.postCode, label: getRequiredOrNot_1.default(t('checkoutPage.step2.form.postCode.label'), true), placeholder: t('checkoutPage.step2.form.postCode.placeholder') })))),
                react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_2.Field, { name: "city", type: "text" }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, disabled: hasPreviousOrder && initialValues.city, label: getRequiredOrNot_1.default(t('checkoutPage.step2.form.city.label'), true), placeholder: t('checkoutPage.step2.form.city.placeholder') }))))),
            react_1.default.createElement("div", { className: "checkout-fieldset" },
                react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_2.Field, { name: "email", type: "email" }, (props) => (react_1.default.createElement(formik_1.CustomInput, { ...props, disabled: true, label: getRequiredOrNot_1.default(t('checkoutPage.step2.form.email.label'), true), placeholder: t('checkoutPage.step2.form.email.placeholder') })))),
                react_1.default.createElement("div", { className: "d-block checkout-field" },
                    react_1.default.createElement(formik_1.CustomMobileInput, { label: getRequiredOrNot_1.default(t('checkoutPage.step2.form.phone.label'), true), disabled: hasPreviousOrder && initialValues.mobile.value, name: "mobile", defaultValue: values.mobile }))))));
};
...
const Step2 = () => {
    const { t } = react_i18next_1.useTranslation();
    const { customerType: customerTypeReducer } = react_redux_1.useSelector((store) => ({
        customerType: store.NEWcustomerType
    }));
    const [customerType, setCustomerType] = react_1.useState(customerTypeReducer || 'COMPANY');
    const { onSubmitForm, getInitialValues, getSchema } = useFormSubmit_1.default();
    const { basket } = react_redux_1.useSelector((store) => ({
        basket: store.basket
    }));
    hooks_1.useDocumentTitle('Check Out Step 2 | Salinaka');
    hooks_1.useScrollTop();
    if (basket.length === 0) {
...
exports.default = WithCheckout_1.default(Step2);
...
const yup_1 = __webpack_require__(87561);
const useFormSubmit = () => {
    const { contactDetails } = react_redux_1.useSelector((store) => ({
        contactDetails: store.NEWcontactDetails
    }));
    const dispatch = react_redux_1.useDispatch();
    const history = react_router_dom_1.useHistory();
    const onSubmitForm = (form, customerType) => {
        dispatch(customerTypeActions_1.setCustomerTypeLocaly(customerType));
        dispatch(contactDetailsActions_1.setContactDetailsLocaly(form));
    ...
    const getSchema = (customerType) => {
        const base = yup_1.object().shape({
            fullname: yup_1.string()
            ...
            mobile: yup_1.object()
                .shape({
                country: yup_1.string(),
                countryCode: yup_1.string(),
                dialCode: yup_1.string(),
                value: yup_1.string()
                    .min(11, 'Numer telefonu powinien składać się z 9 cyfr')
                    .required('Numer telefonu jest wymagany')
            })
                .required('Numer telefonu jest wymagany'),
            street: yup_1.string().required('Adres jest wymagany'),
            postCode: yup_1.string()
                .trim()
                .matches(/^(\d{2})-(\d{3})$/, 'Kod pocztowy powinien mieć format XX-XXX')
                .required('To pole jest wymagane'),
            city: yup_1.string().required('To pole jest wymagane')
        });
...
const preferredDeliveryActions_1 = __webpack_require__(48376);
const yup_1 = __webpack_require__(87561);
exports.formSchema = yup_1.object().shape({
    deliveryType: yup_1.mixed().oneOf(['FREE_PARCEL', 'PICK_UP'], 'Wybierz jedną, aby przejść dalej.')
});
const useFormSubmit = () => {
    const { preferredDelivery } = react_redux_1.useSelector((store) => ({
    ...
    const onSubmitForm = (form) => {
        if (form.deliveryType) {
            dispatch(preferredDeliveryActions_1.setPreferredDelivery(form.deliveryType));
        }
...
const Step3 = ({ total }) => {
    const { contactDetails: { email, mobile, fullname } } = react_redux_1.useSelector((store) => ({
        contactDetails: store.NEWcontactDetails
    }));
    const { onSubmitForm, getInitialValues } = form_1.default();
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useDocumentTitle('Check Out Step 2 | Salinaka');
    hooks_1.useScrollTop();
...
exports.default = WithCheckout_1.default(Step3);
...
const yup_1 = __webpack_require__(87561);
exports.formSchema = yup_1.object().shape({
    street: yup_1.string().required('To pole jest wymagane.'),
    houseNr: yup_1.string().required('To pole jest wymagane.'),
    postCode: yup_1.string().required('To pole jest wymagane.'),
    city: yup_1.string().required('To pole jest wymagane.')
});
const useFormSubmit = () => {
    const { shipping } = react_redux_1.useSelector((store) => ({
        shipping: store.NEWshipping
    }));
    const dispatch = react_redux_1.useDispatch();
    const history = react_router_dom_1.useHistory();
    const onSubmitForm = (form) => {
        dispatch(shippingActions_1.setShippingLocaly(form));
...
exports.default = WithCheckout_1.default(Step3a);
...
const yup_1 = __webpack_require__(87561);
exports.formSchema = yup_1.object().shape({
    paymentType: yup_1.mixed().oneOf(['TRANSFER'], 'Wybierz jedną, aby przejść dalej.')
});
const useFormSubmit = (total, basket, auth) => {
    const history = react_router_dom_1.useHistory();
    const { paymentType, deliveryType, contactDetails, shipping } = react_redux_1.useSelector((store) => ({
    ...
    const dispatch = react_redux_1.useDispatch();
    const onSubmit = (form) => {
        dispatch(prefferedPaymentActions_1.setPreferredPayment(form.paymentType));
        dispatch(checkoutActions_1.addOrderRemotely({
            total,
            basket,
            auth,
...
const Payment = ({ auth, total }) => {
    const { deliveryType, basket } = react_redux_1.useSelector((store) => ({
        deliveryType: store.NEWpreferredDelivery,
        basket: store.basket
    }));
    const { t } = react_i18next_1.useTranslation();
    const { onSubmit, getInitialValues } = form_1.default(total, basket, auth);
    hooks_1.useDocumentTitle('Check Out Final Step | Salinaka');
    hooks_1.useScrollTop();
    // if (!deliveryType) {
...
exports.default = WithCheckout_1.default(Payment);
        ...
        react_1.default.createElement("table", null,
            react_1.default.createElement("tbody", null,
                total.products !== undefined && (react_1.default.createElement("tr", null,
                    ...
                    react_1.default.createElement("td", null,
                        react_1.default.createElement("h4", { className: "basket-total-amount text-subtle text-right margin-0" }, utils_1.displayMoney(total.products))))),
                total.delivery !== undefined && (react_1.default.createElement("tr", null,
                    ...
                    react_1.default.createElement("td", null,
                        react_1.default.createElement("h4", { className: "basket-total-amount text-subtle text-right margin-0 " }, utils_1.displayMoney(total.delivery))))),
                total.sum !== undefined && (react_1.default.createElement("tr", null,
                    ...
                    react_1.default.createElement("td", null,
                        react_1.default.createElement("h2", { className: "basket-total-amount text-right" }, utils_1.displayMoney(total.sum)))))))));
};
...
const WithCheckout = (Component) => react_router_dom_1.withRouter((props) => {
    const path = react_router_dom_1.useLocation();
    const { auth, basket, preferredDelivery } = react_redux_1.useSelector((store) => ({
        auth: store.auth,
        basket: store.basket,
        preferredDelivery: store.NEWpreferredDelivery
    }));
    ...
    const products = utils_1.calculateTotal(basket.map((product) => product.price * product.quantity));
...
const FeaturedProducts = () => {
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useDocumentTitle('Featured Products | Salinaka');
    hooks_1.useScrollTop();
    const { featuredProducts, fetchFeaturedProducts, isLoading, error } = hooks_1.useFeaturedProducts();
    return (react_1.default.createElement(Page_1.default, null,
...
var ProductsTesting_1 = __webpack_require__(31341);
Object.defineProperty(exports, "Products", ({ enumerable: true, get: function () { return __importDefault(ProductsTesting_1).default; } }));
...
const RecommendedProducts = () => {
    const { t } = react_i18next_1.useTranslation();
    hooks_1.useDocumentTitle('Recommended Products | Salinaka');
    hooks_1.useScrollTop();
    const { recommendedProducts, fetchRecommendedProducts, isLoading, error } = hooks_1.useRecommendedProducts();
    return (react_1.default.createElement(Page_1.default, null,
...
const Search = ({ match }) => {
    ...
    react_1.useEffect(() => {
        if (didMount && !store.isLoading) {
            dispatch(productActions_1.searchProduct(searchKey));
        }
    }, [searchKey]);
    react_1.useEffect(() => () => {
        dispatch(miscActions_1.setRequestStatus(''));
    }, []);
...
const Form = ({ product }) => {
    const { t } = react_i18next_1.useTranslation();
    const [variant, setVariant] = react_1.useState(null);
    const { addToBasket, isItemOnBasket } = hooks_1.useBasket();
    ...
    return (react_1.default.createElement(react_1.default.Fragment, null,
        ...
        react_1.default.createElement("h1", null, utils_1.displayMoney(product.price)),
...
const ViewProduct = () => {
    ...
    const { product, isLoading, error } = hooks_1.useProduct(id);
    hooks_1.useScrollTop();
    hooks_1.useDocumentTitle(`View ${product?.name || 'Item'}`);
    const [selectedImage, setSelectedImage] = react_1.useState((product && product.gallery && product.gallery[0].url) || '');
    const { recommendedProducts, fetchRecommendedProducts, isLoading: isLoadingFeatured, error: errorFeatured } = hooks_1.useRecommendedProducts(6);
    react_1.useEffect(() => {
...
`;
const GalleryImage = styled_components_1.default(common_1.ImageLoader) `
  width: 100%;
  height: 100%;
  object-fit: contain;
`;
...
/******/ })()
;
//# sourceMappingURL=main.27b9cf9893f6ebb367cb.js.map